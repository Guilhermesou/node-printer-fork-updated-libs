...
text = text.replace('v8::Local<v8::String>', 'Napi::String')
...
def replace_nan_set(match):
    ...
    if 'V8_STRING_NEW_2BYTES' in val:
        val = re.sub(r'V8_STRING_NEW_2BYTES\((.*?)\)', r'Napi::String::New(env, (char16_t*)\1)', val)
...
text = re.sub(r'return (.*?)\);', r'return \1;', text) # fix the double parenthesis issue
...
text = text.replace('REQUIRE_ARGUMENTS(iArgs, 4);', 'if (info.Length() < 4) { Napi::Error::New(env, "Expected 4 arguments").ThrowAsJavaScriptException(); return env.Null(); }')
...
text = re.sub(r'REQUIRE_ARGUMENT_STRINGW\(iArgs,\s*0,\s*(.*?)\);', r'if (!info[0].IsString()) { Napi::Error::New(env, "Argument 0 must be a string").ThrowAsJavaScriptException(); return env.Null(); }\n    std::u16string \1_u16 = info[0].As<Napi::String>().Utf16Value();\n    auto \1 = \1_u16.c_str();', text)
text = re.sub(r'REQUIRE_ARGUMENT_STRINGW\(iArgs,\s*1,\s*(.*?)\);', r'if (!info[1].IsString()) { Napi::Error::New(env, "Argument 1 must be a string").ThrowAsJavaScriptException(); return env.Null(); }\n    std::u16string \1_u16 = info[1].As<Napi::String>().Utf16Value();\n    auto \1 = \1_u16.c_str();', text)
text = re.sub(r'REQUIRE_ARGUMENT_STRINGW\(iArgs,\s*2,\s*(.*?)\);', r'if (!info[2].IsString()) { Napi::Error::New(env, "Argument 2 must be a string").ThrowAsJavaScriptException(); return env.Null(); }\n    std::u16string \1_u16 = info[2].As<Napi::String>().Utf16Value();\n    auto \1 = \1_u16.c_str();', text)
text = re.sub(r'REQUIRE_ARGUMENT_STRINGW\(iArgs,\s*3,\s*(.*?)\);', r'if (!info[3].IsString()) { Napi::Error::New(env, "Argument 3 must be a string").ThrowAsJavaScriptException(); return env.Null(); }\n    std::u16string \1_u16 = info[3].As<Napi::String>().Utf16Value();\n    auto \1 = \1_u16.c_str();', text)
...
text = text.replace('Nan::Utf8String filenameUtf8(iArgs[0]);', 'std::string filenameUtf8 = info[0].As<Napi::String>().Utf8Value();')
text = text.replace('*filenameUtf8', 'filenameUtf8.c_str()')

# Manual pointer adjustments resulting from Utf16Value changes
text = text.replace('printername.c_str()', 'printername') # if it had been converted
text = text.replace('docname.c_str()', 'docname')
text = text.replace('type.c_str()', 'type')
text = text.replace('(*printername)', '(printername)')
text = text.replace('(*docname)', '(docname)')
text = text.replace('(*type)', '(type)')

text = text.replace('v8::Local<v8::Value> arg0(iArgs[0]);', 'Napi::Value arg0 = info[0];')
