...
class NetworkPrinter extends EventEmitter {
  ...
  /**
   * Scans the local network for available printers on port 9100.
   * 
   * @static
   * @param {object} [options]
   * @param {number} [options.timeout=500] - Timeout for each connection attempt
   * @param {number} [options.concurrency=20] - How many IPs to scan at once
   * @returns {Promise<Array<{host: string, port: number}>>}
   */
  static async discover(options = {}) {
    const { timeout = 500, concurrency = 32 } = options;
    const interfaces = OS.networkInterfaces();
    const targets = [];

    // 1. Identify local subnets
    for (const name of Object.keys(interfaces)) {
      for (const iface of interfaces[name]) {
        // Only IPv4, non-internal (skip 127.0.0.1)
        if (iface.family === 'IPv4' && !iface.internal) {
          const subnet = iface.address.split('.').slice(0, 3).join('.');
          for (let i = 1; i < 255; i++) {
            targets.push({ host: `${subnet}.${i}`, port: 9100 });
          }
        }
      }
    }
    ...
    // 2. Parallel scan with concurrency limit
    ...
    // Process targets in chunks to avoid OS file descriptor limits
    for (let i = 0; i < targets.length; i += concurrency) {
      const chunk = targets.slice(i, i + concurrency);
      await Promise.all(chunk.map(tile => scan(tile)));
    }
